...
public class WebReg {
	...
	public static Course[] selectionSort3(Course[] arr) {
		...
		for (int n=arr.length; n > 1; n--) { 
			...
			if(arr[cc].getDepartment()==arr[cc-1].getDepartment()){
				counter++;
			}
			
			gg = new Course[counter];
			for (int dd=1; dd < n; dd++) { 
				if(arr[dd].getDepartment()==arr[dd-1].getDepartment()){
					gg[dd]=arr[dd];
				}
		...
		for (int n=arr.length; n > 1; n--) {  // n is effective array size
			...
			int maxIndex=0;  // set first item to be max
			for (int j=1; j < n; j++) { 
				if ((gg[j].getCourseNumber() > gg[maxIndex].getCourseNumber())) {
					maxIndex = j;
				}
