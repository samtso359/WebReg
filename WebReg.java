...
public class WebReg {
	...
	public static Course[] commonCourses(Student one, Student two){
		...
		//counting how many non-null class Student one has registered
		...
		//counting how many non-null class Student two has registered
		...
		//copying the registered courses from Student One to dum
		...
		//copying the registered courses from Student Two to dum2
		...
		for(int o = 0; o<= dum.length-1; o++){
			for(int g =0; g<= dum2.length-1; g++){
				if(dum[o].equals(dum2[g])){
					counter++;
				}
			}
			
		}
		System.out.println(counter+ " courses in common");
		if(counter==0) {
